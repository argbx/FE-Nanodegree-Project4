...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    var movingPizzas = document.getElementById('randomPizzaContainer');
    var newwidth = document.getElementById('randomPizzaContainer')[0].style.width;
    var dx = determineDx(movingPizzas[0], size); 
    var pizzaLength = movingPizzas.lenghth;

    for (var i = 0; i < pizzlaLength; i++) {
      movingPizzas.style.width = newwidth;
    }
  }
...
// Moved var pizzasDiv outside the loop .. levent 20151221
...
function updatePositions() {
  ...
  // created var phase to prevent it from being created over and over during the iteration. levent 20151221
  var phase;
  for (var i = 0; i < itemslength; i++) {
    phase = Math.sin((sTop / 1250) + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var cols = 8;
    var s = 256;
// use screen size to calculate number of pizzas on the screen levent-20151221
 var pizzaWidth = Math.ceil(window.innerWidth / s);
 var pizzaHeight = Math.ceil(window.innerHeight / s);
 var total = pizzaWidth * pizzaHeight;
 // declaring elem outside the loop to prevent it from being created during the loop levent 20151221
 var elem;
  var s = 256;
